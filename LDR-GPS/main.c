...
char PowerSaveTimeout = 5;     // Initial value, enable power save mode after 2 min
...
char BootFunc(char input)
{
    ...
    if(enter)
    {
        ...
        LCD_puts_f(PSTR("Jump to bootloader"));
    }
...
char PowerSaveFunc(char input)
{
    ...
    if(enter)
    {
        ...
        LCD_puts_f(PSTR("Press enter to sleep"));
    }
    else if(input == KEY_ENTER)
    {
        PowerSave = TRUE;
...
char AutoPower(char input)
{
    ...
    if(enter)
    {
        ...
        if(AutoPowerSave)  
            ...
            LCD_puts_f(PSTR("Off"));	// mt LCD_puts("Off", 1);
    }
    else if(input == KEY_ENTER)
    {
         enter = 1;
